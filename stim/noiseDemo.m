...
for row = 1:3
    ...
    noiseMatNorm{row}  =  (noiseMat{row} - min(noiseMat{row}(:))) / ...
        (max(noiseMat{row}(:)) - min(noiseMat{row}(:)));
    
    % convolve them with different orientation filter width
    for col = 1:numCol
        kappaPower = linspace(-1, 2, numCol);
        [noiseMatFiltNorm{row,col}, vonMisses{row,col}] = ...
            oriFiltNoise(fftNoiseMat{row}, kappaPower(col));
    end
end
...
for row = 1:3
    ...
    for col = 1:numCol
        noiseMatFiltNorm{row, col} = ...
            imresize(noiseMatFiltNorm{row, col}, noiseImage);
        vonMisses{row, col} = ...
            imresize(vonMisses{row, col}, [noiseImage(1), 1]);
    end
end
