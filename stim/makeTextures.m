...
% Make black noise
black_fix_noise(:, :, 1) = zeros(round(const.noise_size), ...
    round(const.noise_size), 1) + const.background_color(1);
black_fix_noise(:, :, 2) = zeros(round(const.noise_size), ...
    round(const.noise_size), 1) + const.background_color(2);
black_fix_noise(:, :, 3) = zeros(round(const.noise_size), ...
    round(const.noise_size), 1) + const.background_color(3);
...
total_amount  = ((kappa_val_num - 1) * noise_rand_num * stim_ori_num * ...
    bar_step_num_hor) + ((kappa_val_num - 1) * noise_rand_num * ...
    stim_ori_num * bar_step_num_ver) + noise_rand_num * ...
    bar_step_num_hor + noise_rand_num * bar_step_num_ver + 1;
            ...
            for bar_dir = bar_dir_list
                ...
                for bar_step = 1:bar_step_num
                    ...
                    Screen('DrawTextures', scr.main, texs, [], rects, ...
                        angles)
                    Screen('FillRect', scr.main, ...
                        const.background_color, const.left_propixx_hide)
                    Screen('FillRect', scr.main, ...
                        const.background_color, const.right_propixx_hide)
                    Screen('DrawingFinished', scr.main, [], 1);
                                     
                    if const.drawStimuli
                        % plot and save the sreenshot
                        Screen('Flip', scr.main);
                        screen_stim = Screen('GetImage', scr.main, ...
                            const.stim_rect, [], 0, 1);
                    else
                        % save the sreenshot
                        screen_stim = Screen('GetImage', scr.main, ...
                            const.stim_rect, 'backBuffer', [], 1);
                    end
                    screen_filename = sprintf('%s/%sprobe_barStep-%i_kappaStim%i_stimOri-%i_noiseRand%i.mat', ...
                        const.stim_folder, dir_txt, bar_step, ...
                        kappa_val_stim, stim_ori, noise_rand);
...
for noise_rand = 1:noise_rand_num
    ...
    mat_noise_resize = imresize(mat_noise, ...
        round([const.noise_size,const.noise_size]), 'bilinear');
    noise_patch(:, :, 1) = mat_noise_resize * const.stim_color(1);
    noise_patch(:, :, 2) = mat_noise_resize * const.stim_color(2);
    noise_patch(:, :, 3) = mat_noise_resize * const.stim_color(3);
    bar_dir_list = [1, 3];
    ...
    for bar_dir = bar_dir_list
        ...
        for bar_step = 1:bar_step_num
            ...
            Screen('FillRect', scr.main, const.background_color, ...
                const.left_propixx_hide)
            Screen('FillRect', scr.main, const.background_color, ...
                const.right_propixx_hide)
            Screen('DrawingFinished', scr.main, [], 1);
...
if const.drawStimuli
    ...
    % save the sreenshot
    screen_stim = Screen('GetImage', scr.main, const.stim_rect, ...
        'backBuffer', [], 1);
end
