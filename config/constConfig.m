...
const.frame_to_draw_ver = const.bar_step_ver * const.bar_step_dur_ver ...   % number of drawn frame per pass for vertical bar pass
    *const.noise_freq;
const.frame_to_draw_hor = const.bar_step_hor * const.bar_step_dur_hor ...   % number of drawn frame per pass for horizontal bar pass
    * const.noise_freq;
const.frame_to_draw_blk = const.blk_step*const.TR_dur * const.noise_freq;	% number of drawn frame per bar pass for blank

const.num_drawf_max_hor = round(const.bar_step_hor * const.TR_dur / ...     % number of flip per pass in horizontal bar pass
    const.patch_dur);
const.num_drawf_max_ver = round(const.bar_step_ver * const.TR_dur / ...     % number of flip per pass in vertical bar pass
    const.patch_dur);
const.num_drawf_max_blk = round(const.blk_step * const.TR_dur / ...         % number of flip per pass when blank bar pass
    const.patch_dur);
...
const.rand_num_tex = [];
for t = 1:ceil((const.bar_step_hor * const.TR_dur * const.noise_freq)...
        / const.noise_num)
...
const.rand_num_tex_ver = const.rand_num_tex(1:...
    round(const.frame_to_draw_ver));
const.rand_num_tex_hor = const.rand_num_tex(1:...
    round(const.frame_to_draw_hor));
const.rand_num_tex_blk = const.rand_num_tex(1:...
    round(const.frame_to_draw_blk));
...
const.bar_dir_ang = const.bar_dir_ang_start:const.bar_step_dir_ang:...
    (360-const.bar_step_dir_ang+const.bar_dir_ang_start);
...
const.bar_aperture_rect = [scr.x_mid - const.bar_mask_size / 2, ...         % bar native rect
    ...
    scr.y_mid + const.bar_mask_size / 2];
const.bar_step_size_ver = const.stim_size(2) * 2 / ...                      % bar step size in pixels for vertical bar pass
    (const.bar_step_ver - 1);
const.bar_step_size_hor = const.stim_size(1) * 2 / ...                      % bar step size in pixels for horizontal bar pass
    (const.bar_step_hor-1);
...
const.right_propixx_hide = [(scr.scr_sizeX-scr.scr_sizeY) / 2 + ...         % square screen right side hider
    scr.scr_sizeY,...
    0, ...
    scr.scr_sizeX, ...
    scr.scr_sizeY];
...
for tAng = 1:size(const.bar_dir_ang, 2)
    for tSteps = 1:const.bar_step_hor
        const.barCtr_hor(1,tSteps,tAng) = scr.x_mid + ...
            (cosd(const.bar_dir_ang(tAng)) * const.stim_size(1))...
            - (cosd(const.bar_dir_ang(tAng)) * ...
            const.bar_step_size_hor * (tSteps - 1));                        % bar center coord x
        const.barCtr_hor(2, tSteps, tAng) = scr.y_mid + ...
            (-sind(const.bar_dir_ang(tAng)) * const.stim_size(2)) ...
            - (-sind(const.bar_dir_ang(tAng)) * ...
            const.bar_step_size_hor * (tSteps - 1));                        % bar center coord y
        
        const.bar_aperture_rect_hor(:, :, tSteps, tAng) = ...
            CenterRectOnPoint(const.bar_aperture_rect, ...
            const.barCtr_hor(1, tSteps,tAng), ...
            const.barCtr_hor(2, tSteps,tAng));                              % bar rect
        const.bar_aperture_rect_hor(:, :, tSteps, tAng) = ...
            const.bar_aperture_rect_hor(:, :, tSteps, tAng);
    end
end
...
for tAng = 1:size(const.bar_dir_ang, 2)
    for tSteps = 1:const.bar_step_ver
        const.barCtr_ver(1, tSteps, tAng) = scr.x_mid + ...
            (cosd(const.bar_dir_ang(tAng)) * const.stim_size(1)) ...
            - (cosd(const.bar_dir_ang(tAng)) * const.bar_step_size_ver ...
            *(tSteps - 1));                                                 % bar center coord x
        const.barCtr_ver(2, tSteps, tAng) = scr.y_mid + ...
            (-sind(const.bar_dir_ang(tAng)) * const.stim_size(2))...
            - (-sind(const.bar_dir_ang(tAng)) * ...
            const.bar_step_size_ver*(tSteps-1));                            % bar center coord y
        
        const.bar_aperture_rect_ver(:, :, tSteps, tAng) = ...
            CenterRectOnPoint(const.bar_aperture_rect, ...
            const.barCtr_ver(1, tSteps, tAng), ...
            const.barCtr_ver(2, tSteps, tAng));                             % bar rect
        const.bar_aperture_rect_ver(:, :, tSteps, tAng) = ...
            const.bar_aperture_rect_ver(:, :, tSteps, tAng);        
    end
end
...
for tAng = 1:size( const.bar_dir_run, 2)
    for drawf = 1:const.num_drawf_max_hor
        ...
        const.bar_steps_hor(drawf, tAng) = ceil(drawf / ...
            const.bar_step_drawf_hor);
        
        % define trial start & end drawing frames
        if drawf == (const.bar_steps_hor(drawf, tAng)) * ...
                const.bar_step_drawf_hor
        ...
        if drawf >= (const.bar_steps_hor(drawf, tAng)-1) * ...
                const.bar_step_drawf_hor + const.bef_probe_drawf + 1 ...
                &&  drawf <= (const.bar_steps_hor(drawf, tAng) - 1) * ...
                const.bar_step_drawf_hor + const.bef_probe_drawf + ...
                const.probe_drawf && const.bar_dir_run(tAng) ~= 9
        ...
        if drawf >= (const.bar_steps_hor(drawf, tAng) - 1) * ...
                const.bar_step_drawf_hor + const.bef_probe_drawf + 1 && ...
                drawf <= (const.bar_steps_hor(drawf, tAng) - 1) * ...
                const.bar_step_drawf_hor + const.bar_step_drawf_hor && ...
                const.bar_dir_run(tAng) ~= 9
...
for tAng = 1:size( const.bar_dir_run, 2)
    for drawf = 1:const.num_drawf_max_ver
        ...
        % define bar position step number
        const.bar_steps_ver(drawf,tAng) = ceil(drawf / ...
            const.bar_step_drawf_ver);
        
        % define trial start & end drawing frames
        if drawf == (const.bar_steps_ver(drawf, tAng)) * ...
                const.bar_step_drawf_ver
        ...
        % define time probe drawing frames
        if drawf >= (const.bar_steps_ver(drawf, tAng) - 1) * ...
                const.bar_step_drawf_ver + const.bef_probe_drawf + 1 && ...
                drawf <= (const.bar_steps_ver(drawf, tAng) - 1) * ...
                const.bar_step_drawf_ver + const.bef_probe_drawf + ...
                const.probe_drawf && const.bar_dir_run(tAng) ~= 9
        ...
        if drawf >= (const.bar_steps_ver(drawf, tAng) - 1) * ...
                const.bar_step_drawf_ver + const.bef_probe_drawf + 1 && ...
                drawf <= (const.bar_steps_ver(drawf, tAng) - 1) * ...
                const.bar_step_drawf_ver + const.bef_probe_drawf * 2 + ...
                const.probe_drawf && const.bar_dir_run(tAng) ~= 9
...
if const.scanner
    ...
    for bar_pass = 1:const.bar_dir_num
        if const.bar_dir_run(bar_pass) == 9
            ...
            if const.bar_dir_run(bar_pass) == 1 || ...
                    const.bar_dir_run(bar_pass) == 5
                TR_bar_pass = const.bar_step_hor;
            elseif const.bar_dir_run(bar_pass) == 3 || ...
                    const.bar_dir_run(bar_pass) == 7
    ...
    fprintf(1,'\n\tScanner parameters; %1.0f TRs of %1.2f seconds for a total of %s\n', ...
        const.TRs, const.TR_dur, datestr(seconds((const.TRs * ...
        const.TR_dur)), 'MM:SS'));
end
