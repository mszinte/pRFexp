...
% MSBy is set for extended codes       */
  ...
  [keyIsDown,secs,keyCodes] = KbCheck(-1);
  ...
  elseif any(keyCodes(el.f10))
      key=el.F10_KEY;
  elseif any(keyCodes(el.pluskey))
      key=double('+');
  elseif any(keyCodes(el.minuskey))
      key=double('-');
  elseif any(keyCodes(el.returnkey))
      key=el.ENTER_KEY;
  elseif any(keyCodes(el.qkey))
      key=double('q');
  elseif any(keyCodes(el.tkey))
      key=double('t');
  end
